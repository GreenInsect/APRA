...
class Params:
    ...
    # model arch is usually defined by the task 模型结构由数据任务确定
    ...
    attack: str = 'ThrDFed'  # 'ThrDFed' (3DFed), 'ModelReplace' (Model Replacement)
    
    # "Foolsgold", "FLAME", "RFLBAT", "Deepsight", "FLDetector"
    defense: str = "FLDetector"
    ...
    def __post_init__(self):
        ...
        if self.log:
            self.folder_path = f'result/' \
                               f'{self.task}_{self.current_time}_{self.attack}_{self.defense}'
