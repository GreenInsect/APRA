...
# 1. 定义实验参数矩阵
DEFENSES=("apra" "avg" "clip" "deepsight")
...
do
    ...
    do
        ...
        # 5. 检查运行状态
        if [ $? -eq 0 ]; then
            echo "实验成功: [Attack: $ATTACK, Defense: $DEFENSE]"
            # 发送单次实验完成通知（可选，若嫌邮件太多可以注释掉）
            python3 send_mail.py "实验成功: $ATTACK vs $DEFENSE" "配置已生效并运行完成。"
        else
            echo "错误: [Attack: $ATTACK, Defense: $DEFENSE] 运行失败！"
