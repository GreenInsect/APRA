import math
from typing import List, Any, Dict
import torch
import logging
import os
from tqdm import tqdm
from fl_utils.parameters import Params

from dataclasses import dataclass
import torch

from dataclasses import dataclass
from torch.utils.data.dataloader import DataLoader



@dataclass
class FLUser:
    user_id: int = 0
    compromised: bool = False
    train_loader: DataLoader = None


# 每个batch的属性包括id, inputs(输入内容, 比如图片), labels(对应的标签)
@dataclass
class Batch:
    batch_id: int
    inputs: torch.Tensor
    labels: torch.Tensor

    # For PIPA experiment we use this field to store identity label.
    aux: torch.Tensor = None

    def __post_init__(self):
        self.batch_size = self.inputs.shape[0]

    # 放到指定的设备上
    def to(self, device):
        inputs = self.inputs.to(device)
        labels = self.labels.to(device)
        if self.aux is not None:
            aux = self.aux.to(device)
        else:
            aux = None
        return Batch(self.batch_id, inputs, labels, aux)

    def clone(self):
        inputs = self.inputs.clone()
        labels = self.labels.clone()
        if self.aux is not None:
            aux = self.aux.clone()
        else:
            aux = None
        return Batch(self.batch_id, inputs, labels, aux)

    def clip(self, batch_size):
        if batch_size is None:
            return self

        inputs = self.inputs[:batch_size]
        labels = self.labels[:batch_size]

        if self.aux is None:
            aux = None
        else:
            aux = self.aux[:batch_size]

        return Batch(self.batch_id, inputs, labels, aux)

logger = logging.getLogger('logger')
os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'

class FedAvg:
    params: Params
    ignored_weights = ['num_batches_tracked']  # ['tracked', 'running']
    select_clinet: List[FLUser]

    def __init__(self, params: Params) -> None:
        self.params = params

    # FedAvg aggregation
    def aggr(self, weight_accumulator, _):

        for i in range(self.params.fl_no_models):
            updates_name = '{0}/saved_updates/update_{1}.pth'.format(self.params.folder_path, i)
            loaded_params = torch.load(updates_name)
            self.accumulate_weights(weight_accumulator, {key: loaded_params[key].to(self.params.device) for key in loaded_params})

    def get_user_id(self, user):
        self.select_clinet = user

    def accumulate_weights(self, weight_accumulator, local_update):
        for name, value in local_update.items():
            weight_accumulator[name].add_(value)
    
    def get_update_norm(self, local_update):
        squared_sum = 0
        for name, value in local_update.items():
            if 'tracked' in name or 'running' in name:
                continue
            squared_sum += torch.sum(torch.pow(value, 2)).item()
        update_norm = math.sqrt(squared_sum)
        return update_norm

    def fool_accumulate_weights(self, weight_accumulator, local_update, w):
        for name, value in local_update.items():
            weight_accumulator[name].add_(value*w)

    def add_noise(self, sum_update_tensor: torch.Tensor, sigma):
        noised_layer = torch.FloatTensor(sum_update_tensor.shape)
        noised_layer = noised_layer.to(self.params.device)
        noised_layer.normal_(mean=0, std=sigma)
        sum_update_tensor.add_(noised_layer)

    def check_ignored_weights(self, name) -> bool:
        for ignored in self.ignored_weights:
            if ignored in name:
                return True

        return False