...
class FLDetector(FedAvg):
    ...
    def aggr(self, weight_accumulator, global_model: nn.Module):
        ...
        if self.malicious_score.shape[0] > window_size:
            ...
            # Gap statistics
            if gap_statistics(score, num_sampling=20, K_max=10, n=total_participants-len(self.exclude_list)) >= 2:
...
def gap_statistics(data, num_sampling, K_max, n):
        ...
        for k in range(1, K_max + 1):
            k_means = KMeans(n_clusters=k, init='k-means++').fit(data_c+0.0001)
            ...
            v = 0
            for v_kb_i in v_kb:
                v += math.log(v_kb_i+ 1e-8)
            v /= num_sampling
            gap.append(v - math.log(v_k + 1e-8))
            sd = 0
            for v_kb_i in v_kb:
                sd += (math.log(v_kb_i+ 1e-8) - v)**2
